...
function Nexus:Window(config)
    ...
    -- 🔥 FIX: State management dengan proper cleanup
    local SliderState = {
        Dragging = false,
    ...
    -- 🔥 FIX: Unified update function
    local function UpdateSlider(value, animate, triggerCallback)
        ...
        if value == SliderState.LastValue then return end
    ...
    -- 🔥 FIX: Mobile & Desktop compatible input handler
    local function GetInputPosition(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            return input.Position
        elseif input.UserInputType == Enum.UserInputType.MouseMovement or
    ...
    local function CalculateValueFromPosition(position)
        if not SliderTrack or not SliderTrack.Parent then return CurrentValue end
        
        local trackPos = SliderTrack.AbsolutePosition.X
        local trackSize = SliderTrack.AbsoluteSize.X
        
        if trackSize == 0 then return CurrentValue end
        
        local percentage = math.clamp((position.X - trackPos) / trackSize, 0, 1)
        local newValue = Min + (percentage * (Max - Min))
        
        return newValue
    end
    
    -- 🔥 FIX: Input handlers dengan proper touch support
    local function OnInputBegan(input)
        ...
        if position then
            local newValue = CalculateValueFromPosition(position)
            UpdateSlider(newValue, false)
        end
        
        -- Handle glow animation
        if HandleGlow and HandleGlow.Parent then
            Tween(HandleGlow, {
    ...
    local function OnInputChanged(input)
        if not SliderState.Dragging then return end
        
        local inputType = input.UserInputType
        
        if inputType ~= Enum.UserInputType.MouseMovement and 
           inputType ~= Enum.UserInputType.Touch then
            return
        end
    ...
    -- 🔥 FIX: Create invisible button untuk catch inputs
    ...
    -- Connect events
    local beginConn = SliderButton.InputBegan:Connect(OnInputBegan)
    local changeConn = UserInputService.InputChanged:Connect(OnInputChanged)
    local endConn = UserInputService.InputEnded:Connect(OnInputEnded)
    
    table.insert(SliderState.Connections, beginConn)
    table.insert(SliderState.Connections, changeConn)
    table.insert(SliderState.Connections, endConn)
    table.insert(Nexus.Connections, beginConn)
    table.insert(Nexus.Connections, changeConn)
    table.insert(Nexus.Connections, endConn)
    ...
    SliderButton.MouseLeave:Connect(function()
        if not SliderState.Dragging then
    ...
    -- Initialize
    UpdateSlider(CurrentValue, false, false)
    
    -- Register
    if Flag then
        Nexus.Registry[Flag] = {
            Set = function(value, silent)
                UpdateSlider(tonumber(value) or Min, true, not silent)
            end,
            Get = function() return CurrentValue end
        }
    end
    ...
    return {
        ...
        Get = function() return CurrentValue end,
        SetText = function(newText) 
            SliderText.Text = tostring(newText or "") 
        end,
        ...
        Destroy = function()
            -- Cleanup connections
            for _, conn in ipairs(SliderState.Connections) do
                if conn and conn.Connected then
                    conn:Disconnect()
                end
            end
            SliderState.Connections = {}
        end
    }
end
    ...
    -- 🔥 FIX: Safe value update function
    local function UpdateValue(newValue, silent, fromExternal)
        -- Validate numeric input
        if Numeric then
            newValue = tostring(newValue)
            
            -- Allow empty for editing
            if newValue == "" or newValue == "-" or newValue == "." then
                -- Don't update CurrentValue yet
                return
            end
        ...
        -- Avoid unnecessary updates
        if newValue == InputState.LastValue and not fromExternal then
            return
        end
        ...
        -- Update TextBox ONLY if not currently focused
        -- 🔥 KEY FIX: Ini yang bikin text hilang saat edit!
        if not InputState.IsFocused or fromExternal then
        ...
        if not silent then
            pcall(function()
                if Numeric then
                    Callback(tonumber(newValue) or 0)
                else
                    Callback(newValue)
                end
            end)
        ...
        if Flag then
            if Numeric then
                Nexus.Flags[Flag] = tonumber(newValue) or 0
            else
                Nexus.Flags[Flag] = newValue
            end
            Nexus:_ScheduleAutoSave()
        end
    end
    
    -- 🔥 FIX: Numeric input validation (real-time)
    if Numeric then
        TextBox:GetPropertyChangedSignal("Text"):Connect(function()
            if not InputState.IsFocused then return end
            
            local text = TextBox.Text
            
            -- Allow empty, minus, and dot during editing
            if text == "" or text == "-" or text == "." or text == "-." then
                return
            end
            
            -- Remove invalid characters (keep digits, dot, minus)
            ...
            if dotCount > 1 then
                ...
                cleaned = parts[1] .. "." .. table.concat(parts, "", 2)
            end
            
            -- Ensure minus only at start
            local hasMinus = cleaned:sub(1, 1) == "-"
            ...
            if cleaned ~= text then
                ...
                TextBox.CursorPosition = math.min(cursorPos, #cleaned + 1)
            end
        end)
    end
    ...
    TextBox.FocusLost:Connect(function(enterPressed)
        ...
        -- 🔥 FIX: Update value AFTER focus lost
        local finalValue = TextBox.Text
        
        -- Handle numeric empty/invalid
        if Numeric then
            if finalValue == "" or finalValue == "-" or finalValue == "." or finalValue == "-." then
                finalValue = "0"
                TextBox.Text = "0"
            end
        end
        
        UpdateValue(finalValue, false, false)
    end)
