...
function Nexus:Window(config)
    ...
    local function CreateTab(config)
        ...
        function Tab:Collapsible(config)
            ...
            -- Frame Utama (Container Luar)
            local CollapsibleFrame = Create("Frame", {
                BackgroundColor3 = Nexus.Theme.Surface,
                Size = UDim2.new(1, 0, 0, 36), -- Tinggi awal
                ClipsDescendants = true, -- Ini memotong saat animasi
                Parent = TabPage
            })
            ...
            -- [API] Memasukkan Elemen ke dalam Collapsible
            local Group = {}
            
            function Group:Button(cfg)
                -- Reuse fungsi Button tapi ganti Parent-nya
                -- Kita harus membuat ulang logic di sini agar parent ke ContentContainer
            ...
            function Group:Toggle(cfg)
                ...
                local Switch = Create("Frame", {
                    ...
                    Size = UDim2.new(0, 36, 0, 18), Position = UDim2.new(1, -48, 0.5, -9), Parent = ToggleFrame
                })
                AddCorner(Switch, 9)
                
                local Dot = Create("Frame", {
                    BackgroundColor3 = Nexus.Theme.Text, Size = UDim2.new(0, 14, 0, 14),
                    Position = UDim2.new(0, Toggled and 20 or 2, 0.5, -7), Parent = Switch
                })
                AddCorner(Dot, 7)
                
                local Btn = Create("TextButton", {Text="", BackgroundTransparency=1, Size=UDim2.new(1,0,1,0), Parent=ToggleFrame})
                
                local function Update()
                    Tween(Switch, {BackgroundColor3 = Toggled and Nexus.Theme.Accent or Nexus.Theme.Surface}, 0.2)
                    Tween(Dot, {Position = UDim2.new(0, Toggled and 20 or 2, 0.5, -7)}, 0.2)
                    pcall(Callback, Toggled)
                    if Flag then Nexus.Flags[Flag] = Toggled end
                end
                
                Btn.MouseButton1Click:Connect(function() Toggled = not Toggled; Update() end)
                if Flag then Nexus.Registry[Flag] = {Set = function(v) Toggled = v; Update() end, Get = function() return Toggled end} end
